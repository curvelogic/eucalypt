...
;; Operator character class for explicit matching
;; Includes: . ! @ £ % ^ & * | > < / + = - ~ ; ? $ and Unicode operators
(defvar eucalypt--operator-chars "[.!@£%^&*|><\\/+=~;?$∧∨∘¬∸→←⊕⊗⊙⊡⊞⊟≠≡≤≥±×÷√∞]")

(defvar eucalypt-font-lock-keywords
  `(;; c-string (c"...") - highlight entire string including prefix
    ("\\(c\"[^\"]*\"\\)" 1 font-lock-string-face t)
    ;; r-string (r"...") - highlight entire string including prefix
    ("\\(r\"[^\"]*\"\\)" 1 font-lock-string-face t)
    ...
    ;; binary op declaration: (x OP y): - use explicit operator chars
    (,(concat "([[:space:]]*\\sw+[[:space:]]*\\(" eucalypt--operator-chars "+\\)[[:space:]]*\\sw+[[:space:]]*)[[:space:]]*:[[:space:]]") 1 font-lock-function-name-face)
    ;; unary prefix op declaration: (OP x):
    (,(concat "([[:space:]]*\\(" eucalypt--operator-chars "+\\)[[:space:]]*\\sw+[[:space:]]*)[[:space:]]*:[[:space:]]") 1 font-lock-function-name-face)
    ;; zero-arity postfix op declaration: (x OP):
    (,(concat "([[:space:]]*\\sw+[[:space:]]*\\(" eucalypt--operator-chars "+\\)[[:space:]]*)[[:space:]]*:[[:space:]]") 1 font-lock-function-name-face)
    ...
    ;; operators - use explicit character class to include ! and ?
    ;; This matches sequences of operator characters not adjacent to word chars
    (,(concat "\\(?:^\\|[^[:alnum:]_•-]\\)\\(" eucalypt--operator-chars "+\\)\\(?:[^[:alnum:]_•-]\\|$\\)") 1 font-lock-keyword-face)
    ;; intrinsics (__NAME)
    ("__\\sw+" . font-lock-builtin-face)
    ;; quoted intrinsics ('__NAME.METHOD')
    ("'__[^']+'" . font-lock-builtin-face)
    ;; symbols (:name) - use explicit character class for reliable matching
