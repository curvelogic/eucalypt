...
;; Syntax table for tree-sitter mode.  We deliberately do NOT mark " as
;; a string delimiter because tree-sitter handles all string detection,
;; and the syntax table would otherwise interfere with c"..." and r"..."
;; prefix strings by incorrectly pairing quotes.
(defvar eucalypt-ts-mode--syntax-table
  (let ((table (make-syntax-table)))
    ...
    (modify-syntax-entry ?? "w" table)
    table)
  "Syntax table for `eucalypt-ts-mode'.")
...
\\{eucalypt-ts-mode-map}"
  :group 'eucalypt-ts
  :syntax-table eucalypt-ts-mode--syntax-table
