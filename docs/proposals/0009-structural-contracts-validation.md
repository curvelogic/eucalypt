...
- **Spec engine — generalise `match?`, do not replace it.** Lift the existing recursive `mv?`/`mb?`/`ml?` (`lib/prelude.eu:538-553`) into a `check(spec, value) -> Result` that (a) accepts any spec the `to-spec` coercion yields — a value-pattern, a bare predicate, or a parsed `Type` (from `parse.rs`, the type-data an `s"…"` literal evaluates to) — and (b) accumulates a path + reason rather than collapsing to a bool. Medium size; the interpretive core already exists and is tested.
