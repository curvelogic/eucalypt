...
**`as-spec` is the *mechanism*; its *surface* is resolved in the next subsection.** The original function spelling is honest but **magical** in one specific place — its *input*: nothing at the use site signals that the bare symbol `:Server` reaches into another world, the type namespace, so a config author reading `validate(as-spec(:Server), x)` cannot see that a type expression is involved. The fix is to **give the type reference its own syntax** — a `s"…"` literal — while *keeping* `as-spec` as the now un-magical transform from a type to a spec. This supersedes the earlier-considered *reserved type-expression bracket* (`⟬Server⟭`, [0013]'s H12e): a reserved bracket would emit a `Type` node at **parse time** — checker-visible in annotation position, reified in value position, dodging the [0013] phasing and termination hazards — but at the price of carving a delimiter pair out of bracket-space and mode-switching a sub-grammar into the parser, *abandoning the string boundary 0013 concludes to keep for 1.0*. The prefix-string reaches the **same** parse-time `Type` node by adding one variant to the lexer's existing `StringPrefix` family (`c"`/`r"`/`t"`; `src/syntax/rowan/lex.rs:36-43`) — no new grammar, and the type content stays a string, so TS-A7 alias tooling is untouched. **The bridge is therefore written `as-spec(s"Name")`** — or just `s"Name"` wherever a spec is expected, via the auto-lowering of the next subsection. The bare-symbol `as-spec(:Name)` used in the motivation above is shorthand for it; everything from the *Resolution* onward uses the explicit form.
...
### Resolution: `s"…"` is type-data; `as-spec` lowers it to a spec

**The resolution separates representation from lowering.** The bare-symbol `as-spec(:Server)` was magical in exactly one place — its *input*: a symbol silently meaning a type. The fix splits the two jobs that surface bundled. `s"…"` is a **literal for the type's *data representation*** — it evaluates to ordinary eucalypt data describing the type, the type-DSL string being only the concise, parse-time-checked *spelling* of that data (the same notation the checker reads in annotations). `as-spec` is then the **transform from that type-data to an executable *spec value*** — the predicate-with-blame `validate`/`valid?` run — now un-magical because its input is an explicit `s"…"`, not a bare symbol. Two stages, *represent* then *lower*; `as-spec` is **kept, de-magicked, not retired**. Position decides only annotation-read versus runtime-value: in **annotation** position the checker reads `s"…"` as a type and it erases; in **value** position it evaluates to type-data. The type→*spec* crossing is always the explicit `as-spec` (or the auto-lowering below) — never silent, never merely positional.

**Why a string prefix, not a keyword or a bracket — non-negotiable #1 intact.** eucalypt already lexes a `StringPrefix` family — `c"…"`, `r"…"`, and the ZDT literal `t"…"` (`src/syntax/rowan/lex.rs:36-43`, detected at `:259-298`; `t"…"` validated to a `DateTime` at `src/syntax/rowan/kind.rs:106`). `s"…"` adds **one variant to that enum and one lexer branch** — no new keyword, no new operator, no new block construct, so house-style non-negotiable #1 holds, with three-fold precedent. It is strictly *less* of a step than [0013]'s H12e bracket: the type content **stays inside a string** — exactly the boundary 0013 concludes to keep for 1.0, so TS-A7 span-indexed alias tooling is untouched — whereas the bracket abandons it. It follows the `t"` (ZDT) model — the *only* prefix that does so today, since `r"` and `c"` keep interpolation **on** (`r"` is grouped with plain strings, `{{`/`}}` as escapes; `lex.rs:388-398`). Like `t"`, `s"` lexes with **interpolation off**, emitting its content as a single opaque token (`t"` early-returns at `:371-372`) which is parsed to a typed value at parse time — `t"` to a `DateTime`, `s"` to a `Type`. That both hands the type-DSL parser one clean token and, as a by-product, retires the `{{..}}` doubling the type-string spelling needs today (`Server: "{{ host: string }}"`, above): a literal `s"{host: string}"` is no longer an interpolation site.
...
**What `s"…"` evaluates to: type-data — not a spec, and not a new `Native`.** In value position `s"…"` yields ordinary, **serialisable** eucalypt data describing the type (a tagged shape such as `{ kind: :arrow, from: …, to: … }`) — introspectable, comparable and exportable through the machinery blocks already use, so it needs **no** new `Native` variant (`src/eval/memory/syntax.rs:37-58`). It is deliberately an AST-as-data: that is the representation of a *type*, distinct from the spec `as-spec` produces from it. (An earlier draft of this section argued `s"…"` should reify straight into the `match?` vocabulary "not a runtime AST"; that collapsed the two stages and is what manufactured the `[T]` and serialisation muddles. The type-data *is* the AST-as-data — correct, because it is not a competing *spec* representation but the *type* representation, with `as-spec` the single bridge to the one spec form. The `match?` vocabulary is what `as-spec` *produces*, not what `s"…"` *is*.)

**`as-spec` lowers type-data to a spec; consumers auto-lower.** Every spec consumer (`validate`/`valid?`/`match?`/`conform`/`gen`/`refine`) normalises its argument through one `to-spec` coercion: a bare predicate and a value-pattern are *already* specs; **type-data is lowered by `as-spec`**. So `validate(s"Server", x)` works directly, while `as-spec` remains the same transform exposed by name when a spec is wanted as a first-class, reusable value (`srv: as-spec(s"Server")`). Because `s"…"` is a **literal**, that implicit `as-spec` is the **compile-time** reification the Implementation sketch already prefers — folded once into ordinary core, with no per-call cost. The single new obligation auto-lowering imposes is that `to-spec` distinguish type-data from a value-pattern block; for an `s"…"` *literal* this is static (the literal site is syntactically distinct) and therefore free, and only *computed* type-data flowing into a consumer needs an explicit `as-spec` (or a tagged representation) — the same literal-versus-computed boundary as alias resolution above.

**The lowering is the single place run-time meaning is assigned — so `[T]` does not "collide".** All execution semantics are baked in `as-spec`'s lowering of the type-data: a record becomes an (open or closed) block check — a closed/row record adding a no-extra-keys assertion on top of `match?`'s open block rule (`lib/prelude.eu:545-546`) — `[T]` becomes "a list, every element conforming to `T`" (any length), a union an `or`, a refinement an attached predicate, and a function arrow **nothing** (arrows have no runtime spec and erase, so `s"Server -> number"` contributes only its structural parts in value position). This is why the type-DSL `[T]` (any length) and a `match?` value-pattern `[p]` (a one-element positional list, `:551-552`) never conflict: they are *different surfaces* — type-data versus value-pattern — lowered by *different* `to-spec` rules; they share the `[…]` glyph but never one grammar, leaving only the mild, documentable fact that the two notations spell lists differently. The trade-off stays explicit for review: 0009 originally prized adding *no* syntax, and this accepts one new string-prefix (not a keyword) to de-magic the type reference and retire `{{..}}`.
...
valid-server: s"Server" refine { port: (_ > 0) ∧ (_ < 65536), replicas: (_ >= 1) }
```
...
Deliberately, refinements are **not** attached to the type-alias name. The same object legitimately needs **different specs in different contexts** — a lenient ingress spec, a stricter export spec, a test spec — so binding one refinement set to `:Server` would conflate "the structural type" with "one particular contract on it". Instead `refine` produces *distinct, separately-named* spec values (`ingress-server`, `export-server`, …) that all share the single structural core via `s"Server"`. The type is defined once and stays a clean structural mirror; the contracts are as many as the contexts demand, each a named value — never an attempt to push predicates into the erased type, which the asymmetry forbids.
...
config: imported-data validate(s"Server")   # blame points at the source, not a deep use
...
The proposal keeps that working verbatim, and makes the value-pattern `{host: string?, port: number?}` and the reified alias `as-spec(s"Server")` *both* spec values usable wherever a spec is expected — `validate`, `valid?`/`match?`, `conform` — while the alias additionally serves as a static type. The bare symbol `:Server` is untouched: still a literal value, never silently a shape.
...
- **Spec engine — generalise `match?`, do not replace it.** Lift the existing recursive `mv?`/`mb?`/`ml?` (`lib/prelude.eu:538-553`) into a `check(spec, value) -> Result` that (a) accepts any spec the `to-spec` coercion yields — a value-pattern, a bare predicate, or a parsed `Type` (from `parse.rs`, the type-data an `s"…"` literal evaluates to) — and (b) accumulates a path + reason rather than collapsing to a bool. Medium size; the interpretive core already exists and is tested. Optional record fields (`name?:`, [0021](0021-optional-record-fields.md)) add one arm to the field rule `me?`: present ⇒ match, absent ⇒ pass.
- **`as-spec` / `to-spec` — the type-data → spec bridge.** An `s"…"` literal parses to a `Type` (type-data) at parse time, resolving any alias names against the alias map; `as-spec` (the explicit form) and the `to-spec` coercion consumers apply both lower that `Type` to a spec value. Preferably a **compile-time** pass — the `s"…"` argument is a literal, so its lowering folds then — so it runs independently of `--type-check` and retains no runtime type-DSL parser; this is the single crossing between the type and value worlds, and a bare symbol is never interpreted as anything but a literal value. Everything downstream is ordinary core.
...
- **Registry** — alias-name resolution (performed inside `s"…"`) over the existing alias/`types:` mechanism; no new global state if alias references suffice.
...
- **Ease-of-definition regresses under unification.** If unifying with the type DSL makes the cheap value-pattern spelling *harder* to write, the proposal defeats itself. *Falsifier:* the simplest spec a user writes is longer or more ceremonious than today's `match?` pattern. *Guard:* the value-pattern spelling must remain a no-annotation path; the type-DSL string is an *opt-in* for static reach, never a requirement. Crossing the type/value boundary is the only place a cost can land, and auto-lowering keeps it to the `s"…"` literal itself: `validate(s"Server", x)` needs no extra wrapping call, and because the literal folds at compile time it is not even a runtime cost — an explicit `as-spec` is reached for only to name a spec value for reuse. Writing a plain value-pattern pays nothing.
