...
config: imported-data validate(:Server)   # blame points at the source, not a deep use
...
The proposal keeps that working verbatim, and makes `{host: string?, port: number?}`, `"{host: string, port: number}"`, and a named `:Server` *the same spec* — usable as a type annotation, a `validate` argument, or a `match?` pattern interchangeably.
