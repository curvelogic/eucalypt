...
Every channel through which external data enters the program is, by construction, `any`-typed:

- **`parse-as`** parses a string of structured data and returns eucalypt data, "parsed as inert data" (`lib/prelude.eu:1101-1103`). Its intrinsic `__PARSE_STRING` is typed `(symbol, string) -> any` (`src/eval/intrinsics.rs:828-829`). Nothing downstream knows the shape.
...
So the type system genuinely **stops at the I/O boundary**: `any` in, `string` out, and the checker stays silent in between, by design ([0002]; `subtype.rs:181`, `unify.rs:56`).

**And the consequence of a silent shape error is unusually expensive,** because `lookup` *raises* rather than returning null. `lookup(s, b)` is documented "error if not found" (`lib/prelude.eu:410-412`); the existence of `lookup-or` with an explicit default (`lib/prelude.eu:418-420`) is the tell, and at the STG level a missing key compiles to `LOOKUP_FAIL`, which produces "Key not found: {key}" (`src/eval/stg/block.rs:1735-1751`). So a manifest assembled from data whose shape is *slightly* wrong — a renamed upstream key, a string where a number was expected — does not render a wrong-but-plausible artefact; it aborts deep in evaluation, at the *use* site, with an error naming the symbol but not the *source* of the bad data. The user learns `:replicas` is missing; not that `config.yaml` omitted it. The gap between "where the data was wrong" and "where evaluation noticed" is the whole problem.

Today the only tools are `assert(p?, s, v)` (`lib/prelude.eu:1218-1219`, a thin `panic` wrapper) and the test-only `//=` / `//!` expectation operators (`lib/prelude.eu:1228-1258`). Neither is a *schema*. There is no way to say, once, at ingress: *this data must look like this, and if it does not, tell me precisely how it failed and where.*
...
**A contract is a shape, evaluated as a runtime predicate-with-blame over data.** The design has one principle: *do not make users learn a second shape language*. Eucalypt already has a shape vocabulary — the type DSL parsed by `src/core/typecheck/parse.rs`, which today expresses records (open/closed/row — `parse.rs:545`, `types.rs:128`), unions (`Type::Union` — `types.rs:140`), and literal symbols (`Type::LiteralSymbol` — `types.rs:146`, `parse.rs:450`), alongside `[T]`, tuples, and the `IO`/`Lens` constructors. The Stage-A/B roadmap extends this vocabulary precisely where validation wants it: `Dict(T)` for homogeneous blocks (H19) and the finite refinements of H7 — `NonEmpty`, `Positive`, `Nonzero`. The contract system **reads the same shapes**. A shape that the static checker uses for advice, the contract engine uses for enforcement.
...
- **Do nothing; rely on `assert` + `//=`.** Rejected: `assert` is a `panic` wrapper (`lib/prelude.eu:1218`) with no structure, and `//=` is test-only. Neither is a schema; neither tells the user *where the data was wrong*.
- **A separate schema language (JSON Schema-style).** Rejected: it forces users to learn a second shape vocabulary divorced from the type DSL they already read, and it cannot share the H7/H19 extensions. Structural contracts in the native DSL keep one vocabulary.
...
- **Static-only refinements (H7 without runtime checks).** Insufficient: a static `Positive` cannot be proven of `any`-typed external input, which is the *only* case that matters here.
...
- H7's `NonEmpty`/`Positive` refinements gain a runtime consumer; the same shape serves `eu check` advice and `validate` enforcement with no second vocabulary.
...
**Eucalypt source.** `src/eval/intrinsics.rs:823-824` (`RENDER_TO_STRING : (any, symbol) -> string`), `:828-829` (`PARSE_STRING : (symbol, string) -> any`), `:853-854` (`EXPECT`), `:862` onward (predicate intrinsics); `lib/prelude.eu:410-412` (`lookup` raises), `:418-420` (`lookup-or`), `:1101-1103` (`parse-as` inert data), `:1097-1099` (`render-as`), `:2152-2175` (`parse-args`, coercion via `parse-as`), `:1218-1219` (`assert`), `:1228-1258` (`//=`, `//=?`, `//!`), `:376-392` (`block?`/`list?`/`number?`/… predicates); `src/eval/stg/block.rs:1735-1751` (`LOOKUP_FAIL` "Key not found"); `src/core/typecheck/parse.rs:316, 450, 545` (union / literal-symbol / record grammar); `src/core/typecheck/types.rs:128, 140, 146` (`Record`, `Union`, `LiteralSymbol`); `src/eval/error.rs:5, 277-287` (`codespan_reporting` diagnostics, `help:`, "available keys"); `src/import/` (untyped data ingress).

**Eucalypt docs.** `docs/guide/type-checking.md:1-6, 42-45` (advisory checker; `any` from runtime values); `docs/guide/testing.md` (`//=`/`//!` harness); `docs/development/type-system-evolution.md` H7 (refinements), H13c (opt-in casts), H19 (`Dict`), §5 questions 6 & 7; `docs/proposals/_house-style.md` §1 non-negotiables.
