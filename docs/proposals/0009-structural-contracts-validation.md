...
**`as-spec` is the *mechanism*; its *surface* is an open question for [0013].** The function spelling is honest but **magical**: nothing at the use site signals that `:Server` reaches into another world — the type namespace — so a config author reading `validate(as-spec(:Server), x)` cannot see that a type expression is involved. The cleaner end-state, which this proposal flags rather than settles, is to **promote type references to syntax**: a *reserved* type-expression bracket (a dedicated delimiter — explicitly **not** the user-definable idiot brackets `⟦⟧`, and glyph/ASCII-fallback TBD) into which the lexer mode-switches the type sub-grammar, so `⟬Server⟭` replaces `as-spec(:Server)` and an inline `⟬{host: string, port: number}⟭` replaces `{{..}}`-escaped type strings. This is neither of the embedding roads [0013] currently rejects: not idiot-bracket embedding (those desugar to runtime functions the checker cannot see), nor first-class-type-values (arbitrary values, with a termination problem). A *reserved* bracket emits a `Type` node at **parse time** — checker-visible in annotation position, compile-time-reified in value position — dodging both the phasing and the termination hazards. It also makes the asymmetry **visible on the page**: everything *inside* the brackets is the type world (dual-use, structural); everything *outside* (`refine { port: (_ > 0) }`) is the value world (runtime-only predicates). Whether to add the delimiter at all — and which glyph — is [0013]'s call; it is precisely that proposal's §9 evidence-trigger firing ("users need to reference type aliases as values"). 0009 commits only to the *mechanism* (reification): `as-spec` is its no-new-syntax surface, the reserved bracket its intended de-magicked one.
