...
- `6f005e44` Phase 2 — **`step` dispatch + `handle_op` — FIRST END-TO-END EXECUTION**. `step`: Native→return_native / Closure arity>0→return_fun / arity0→handle_op. `handle_op` arms: Atom(L/G/V), Cons, Case, Seq, Ann, BlackHole. Two end-to-end tests run through `step`: `atom(42)`→Native 42; `case nil→7`. STUBBED arms: App/DirectApp/Bif/Let/LetRec/Meta/DeMeta/LookupLit. NOT YET: thunk memoisation (local thunk entered = re-evaluated, no blackhole/Update — needs blackhole template + Update push, lands with App increment).
