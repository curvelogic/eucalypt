# Summary

[Welcome](welcome/index.md)

# Getting Started

- [What is Eucalypt?](welcome/what-is-eucalypt.md)
- [Quick Start](welcome/quick-start.md)
- [Eucalypt by Example](welcome/by-example.md)

# The Eucalypt Guide

- [Blocks and Declarations](guide/blocks-and-declarations.md)
- [Expressions and Pipelines](guide/expressions-and-pipelines.md)
- [Lists and Transformations](guide/lists-and-transformations.md)
- [Strings and Text](guide/string-interpolation.md)
- [Functions and Combinators](guide/functions-and-combinators.md)
- [Operators](guide/operators.md)
- [Anaphora (Implicit Parameters)](guide/anaphora.md)
- [Block Manipulation](guide/block-manipulation.md)
- [Imports and Modules](guide/imports-and-modules.md)
- [Working with Data](guide/working-with-data.md)
- [The Command Line](guide/command-line.md)
- [YAML Embedding](guide/yaml-embedding.md)
- [Testing with Eucalypt](guide/testing.md)
- [Date, Time, and Random Numbers](guide/date-time-random.md)
- [IO and Shell Commands](guide/io.md)
- [Monads and the monad() Utility](guide/monads.md)
- [Advanced Topics](guide/advanced-topics.md)

# Reference

- [Language Syntax Reference](reference/syntax.md)
- [Operator Precedence Table](reference/operators-and-identifiers.md)
- [Prelude Reference](reference/prelude/index.md)
  - [Lists](reference/prelude/lists.md)
  - [Blocks](reference/prelude/blocks.md)
  - [Strings](reference/prelude/strings.md)
  - [Numbers and Arithmetic](reference/prelude/numbers.md)
  - [Booleans and Comparison](reference/prelude/booleans.md)
  - [Combinators](reference/prelude/combinators.md)
  - [Calendar](reference/prelude/calendar.md)
  - [Sets](reference/prelude/sets.md)
  - [Random Numbers](reference/prelude/random.md)
  - [Metadata](reference/prelude/metadata.md)
  - [IO](reference/prelude/io.md)
  - [N-Dimensional Arrays](reference/prelude/arrays.md)
- [CLI Reference](reference/cli.md)
- [Import Formats](reference/import-formats.md)
- [Export Formats](reference/export-formats.md)
- [Error Messages Guide](reference/error-messages.md)
- [Agent Reference](reference/agent-reference.md)

# Understanding Eucalypt

- [Design Philosophy](understanding/philosophy.md)
- [Lazy Evaluation](understanding/lazy-evaluation.md)
- [Architecture](architecture.md)
- [Eucalypt Style](eucalypt-style.md)

# FAQ

- [Frequently Asked Questions](faq.md)

# Appendices

- [Syntax Cheat Sheet](appendices/cheat-sheet.md)
- [Syntax Gotchas](appendices/syntax-gotchas.md)
- [Migration from v0.2 to v0.3](appendices/migration.md)
