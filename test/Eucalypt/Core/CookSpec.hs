...
cookSpec =
  describe "cooking op soup" $ do
    ...
    it "handles unary prefix" $
      cookUp [pre100, int 10] `shouldBe` (Right $ app (bif "PRE100") [int 10])
    it "handles unary postfix" $
      cookUp [int 10, post100] `shouldBe` (Right $ app (bif "POST100") [int 10])
    ...
    it "substitutes anaphoric lambda for empty" $
      cookUp [] `shouldBe` (Right $ lam ["_0"] $ var "_0")
    it "corrects pre10 pre10 pre10 pre10 with anaphor and abstracts" $
      ...
      lam ["_0"] $
       app
         (bif "PRE10")
         [app (bif "PRE10") [app (bif "PRE10") [app (bif "PRE10") [var "_0"]]]])
