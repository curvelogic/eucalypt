...
/// was built alongside, with no `EU_PREDECODE` override. The one non-default
/// dispatch path left (`EU_PREDECODE=0`, byte-dispatch — HeapSyn, the other
/// one this note used to also cover, was deleted by the Phase 4 collapse,
/// eu-oufc) is a separate, already-tracked issue (eu-l51r7): asserted
/// nowhere in this corpus, and it diverges from these same goldens on a
/// further, disjoint set of fixtures. Recorded by hand (not computed from a
/// live run of every dispatch path) because capturing that would need
/// another full corpus sweep; update the count here whenever eu-l51r7's own
/// inventory moves. Tagged eu-1hcw: when that bead deletes the byte-dispatch
/// path, this note (and eu-l51r7's remaining half) goes with it.
fn engine_scope_note() -> String {
    "\n## Engine scope\n\n\
     ...
     gate exercises with a real blob. It says nothing about the non-default \
     byte-dispatch path.\n\n\
     `EU_PREDECODE=0` is not asserted against these goldens at all, and it \
     diverges from them on its own set of fixtures — a separate, \
     already-known issue, not part of the count above (eu-l51r7; eu-1hcw \
     tracks this path's eventual deletion). As of eu-l51r7's most recent \
     count: 6 fixtures diverge under `EU_PREDECODE=0`. In every case the \
     difference is one `while evaluating (outermost first):` note frame \
     missing, duplicating the primary label's own file:line:col — never a \
     wrong location or a missing primary. See eu-l51r7 for the full \
     inventory; the owner ruled this does not block 0.14 (P2, characterised \
