...
- **BV1 Phase 4 collapse: deleted the HeapSyn tree-walk engine, simplified to the single bytecode engine (eu-oufc)** — owner-authorised 2026-08-04 once the eu-7oshh A/B study (PR #1116, `docs/superpowers/reports/2026-08-03-engine-ab-study.md`) confirmed the bytecode engine at parity or ahead everywhere that mattered. Deleted `src/eval/machine/vm.rs`'s `Machine`/`MachineState`/`MachineBifContext`/`HeapNavigator` (kept only the SIGINT flag), `src/eval/memory/loader.rs`, the `HeapSyn` enum and its `GcScannable`/`Repr` impls in `src/eval/memory/syntax.rs`, `machine::env_builder`/`machine::cont`, and the `EU_HEAPSYN`/`EU_BYTECODE` engine-select flags. Trimmed (not deleted) `StgBuilder` to its three engine-neutral methods (`str`/`str_ref`/`sym_ref`) and the `IntrinsicMachine` trait's ~20 code-type-neutral ABI methods to abstract (no HeapSyn-default body) now that the bytecode engine is the sole implementor; `GcScannable` itself is retained — it still serves the bytecode engine's own closures, env frames and continuations. Ported the ~46 `stg::testing::machine()`-based unit tests (arith/block/eq/force/render/string) onto a `BytecodeMachine`-backed harness, and the WASM evaluation pipeline (`src/wasm_pipeline.rs`) and two criterion benchmarks (`benches/alloc.rs`, `benches/gc.rs`) onto bytecode-native primitives. Reconciled six differential-comparison test files that spawned a second `EU_HEAPSYN=1` process to compare against the default: deleted the ones whose entire purpose was bc-vs-hs equivalence, re-axised the ones guarding a still-real property (fork-path/GC-safety) onto the `EU_PREDECODE=0` byte-dispatch path instead, and converted `tests/bytecode_io_differential_test.rs` to direct assertions on the bytecode driver's output (incidentally fixing a latent bug where one of its fixtures used non-existent `where`-clause syntax and was comparing two parse errors). `cargo xtask engine-ab`'s live A/B run now refuses with a clear message rather than silently comparing bytecode to itself; `--check` and the `results.jsonl` ledger are unchanged. A post-collapse redesign of that tooling for the predecoded-vs-byte-dispatch axis is tracked by eu-hn3j0. Net effect: ~8,600 fewer lines in the tree. Verified under `EU_GC_VERIFY=2`/`EU_GC_POISON=1`, full `cargo test`, and the source-prelude tick-parity tripwire
...
- **`eu error EU-RENDER-SHAPE` described only the html shape case, not non-text block keys (eu-1tkk.7.46)** — eu-1z503 extended `EU-RENDER-SHAPE` to also cover a block key that JSON/TOML/`eu` output cannot carry as text, but updated **neither** catalogue — its commits touch `src/export/` and test fixtures only. `docs/reference/error-codes.md` was brought up to date later, by eu-v129i's release-notes pass; the `eu error` catalogue in `src/driver/error_codes.rs` still described the html case alone. Both catalogues now cover both cases, and a new unit test asserts the two files document the same set of `EU-*` codes — it cannot catch stale prose (the two are deliberately different in form: markdown with links vs. plain terminal text), but it catches a code added to one and not the other, fault-injection verified by deleting the `EU-RENDER-SHAPE` entry from one side
