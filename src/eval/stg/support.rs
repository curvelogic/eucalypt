...
impl Iterator for DataIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
            ...
            HeapSyn::Cons { tag, args } => match (*tag).try_into() {
                ...
                _ => {
                    return Some(Err(ExecutionError::Panic(
                        "expected a list but found a different data type".to_string(),
                    )))
                }
            },
            _ => {
                return Some(Err(ExecutionError::Panic(
                    "expected a list but found a non-data value".to_string(),
                )))
            }
        };
        ...
        let head = match h_ref {
            ...
            None => {
                return Some(Err(ExecutionError::Panic(
                    "malformed list: missing head element".to_string(),
                )))
            }
        };
        ...
        match t_ref {
            ...
            None => {
                return Some(Err(ExecutionError::Panic(
                    "malformed list: missing tail element".to_string(),
                )))
            }
        }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let (h_ref, t_ref) = match &*code {
            HeapSyn::Cons { tag, args } => match (*tag).try_into() {
                ...
                _ => {
                    return Some(Err(ExecutionError::Panic(
                        "expected a string list but found a different data type".to_string(),
                    )))
                }
            },
            _ => {
                return Some(Err(ExecutionError::Panic(
                    "expected a string list but found a non-data value".to_string(),
                )))
            }
        };
        ...
        let native = match h_ref {
            ...
            None => {
                return Some(Err(ExecutionError::Panic(
                    "malformed string list: missing head element".to_string(),
                )))
            }
        };
        ...
        match t_ref {
            ...
            None => {
                return Some(Err(ExecutionError::Panic(
                    "malformed string list: missing tail element".to_string(),
                )))
            }
        }
        ...
        if let Native::Str(s) = native {
            ...
            Some(Err(ExecutionError::Panic(
                "expected a string in list but found a different type".to_string(),
            )))
        }
    }
}
