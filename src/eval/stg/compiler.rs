...
impl<'rt> Compiler<'rt> {
    ...
    /// Compile a block
    ///
    /// Emits a cons-list `Block` data constructor from the compiled
    /// key-value pairs. Block literals stay as standard cons-list blocks;
    /// persistent blocks are constructed explicitly via `PBLOCK_FROM_BLOCK`.
...
pub mod tests {
    ...
    pub fn test_compile_block() {
        ...
        // compile_block emits a cons-list Block data constructor.
        // Persistent blocks are constructed explicitly via PBLOCK_FROM_BLOCK.
        let syntax = dsl::letrec_(
            vec![
