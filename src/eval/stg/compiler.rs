    ...
    pub fn to_diagnostic(&self, source_map: &SourceMap) -> Diagnostic<usize> {
        ...
        match self {
            CompileError::FreeVar(_, name) => {
                // Translate internal desugarer names for expression anaphora to a
                // user-friendly message.  Names of the form '_e_a[N]', '_e_n0',
                // '_e_il0' etc. are generated when anonymous parameters ('_') are
                // cooked; they are meaningless to the user.
