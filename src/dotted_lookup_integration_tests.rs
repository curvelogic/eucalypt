...
mod tests {
    ...
    fn test_simple_dotted_lookup_parsing() {
        ...
        assert_eq!(syntax.kind(), crate::syntax::kind::SyntaxKind::SOUP);
        ...
        assert!(matches!(
            elements[0],
            crate::syntax::ast::Element::Block(_)
        ));

        // Second element should be a name with dot operator
        if let crate::syntax::ast::Element::Name(name) = &elements[1] {
            if let Some(crate::syntax::ast::Identifier::OperatorIdentifier(op)) =
                name.identifier()
            {
                assert_eq!(op.text(), ".");
            }
        ...
        if let crate::syntax::ast::Element::Name(name) = &elements[2] {
            if let Some(crate::syntax::ast::Identifier::NormalIdentifier(normal)) =
                name.identifier()
            {
                let full_id =
                    crate::syntax::ast::Identifier::NormalIdentifier(normal.clone());
    ...
    fn test_interpolation_expression_capabilities() {
        ...
        use crate::syntax::string_lex::StringPatternLexer;
