...
compile :: Eq v => Int -> (v -> Ref) -> Maybe Ref -> C.CoreExp v -> StgSyn
...
-- | Empty list
compile _ _ _ (C.CoreList []) = appcon_ stgNil mempty

-- | List literals
compile envSize context _metaref (C.CoreList els) = let_ elBinds buildList
  where
