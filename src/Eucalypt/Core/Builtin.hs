...
euPanic _ [e@(CorePrim (CoreString s))] = throwEvalError $ Panic s (CoreExpShow e)
euPanic _ as = throwEvalError $ Bug "Bad arguments for panic" (CoreExpShow (CoreList as))
...
euEq _ as = throwEvalError $ Bug "__EQ called with bad args" (CoreExpShow (CoreList as))
...
evalBoolean whnfM expr =
  whnfM expr >>= \case
      ...
      _ -> throwEvalError $ NotBoolean (CoreExpShow expr)
...
euIf _ as = throwEvalError $ Bug "__IF called with bad args" (CoreExpShow (CoreList as))
...
euNot _ as = throwEvalError $ Bug "__NOT called with bad args" (CoreExpShow (CoreList as))
...
euAnd _ as = throwEvalError $ Bug "__AND called with bad args" (CoreExpShow (CoreList as))
...
euOr whnfM [l, r] = do
  ...
  rbool <- evalBoolean whnfM r
  return $ CorePrim (CoreBoolean (lbool || rbool))
euOr _ as = throwEvalError $ Bug "__OR called with bad args" (CoreExpShow (CoreList as))
...
arith whnfM op [l, r] = do
  ...
  case (l', r') of
    ...
    (i, j) -> throwEvalError $ NotNumber (CoreExpShow (CoreList [i, j]))
arith _ _ as = throwEvalError $ Bug "Arith op called with bad args" (CoreExpShow (CoreList as))
...
euDiv whnfM [l, r] = do
  ...
  case (l', r') of
    ...
    _ -> throwEvalError $ NotNumber (CoreExpShow (CoreList [l', r']))
  where
    check val =
      if isInfinite val
        then throwEvalError $ DivideByZero (CoreExpShow r)
        else return $ CorePrim (CoreFloat val)

euDiv _ as = throwEvalError $ Bug "Division with bad args" (CoreExpShow (CoreList as))
...
arithComp whnfM op [l, r] = do
  ...
  case (l', r') of
    ...
    (i, j) -> throwEvalError $ NotNumber (CoreExpShow (CoreList [i, j]))
arithComp _ _ as = throwEvalError $ Bug "Comparison op called with bad args" (CoreExpShow (CoreList as))
...
euHead whnfM [l] =
  whnfM l >>= \case
    ...
    (CoreList []) -> throwEvalError $ EmptyList (CoreExpShow (CoreList [] :: CoreExpr))
    e -> throwEvalError $ NotList (CoreExpShow e)
euHead _ as = throwEvalError $ Bug "__HEAD called with bad args" (CoreExpShow (CoreList as))
...
euTail whnfM [l] =
  whnfM l >>= \case
    ...
    (CoreList []) -> throwEvalError $ EmptyList (CoreExpShow (CoreList [] :: CoreExpr))
    e -> throwEvalError $ NotList (CoreExpShow e)
euTail _ as = throwEvalError $ Bug "__TAIL called with bad args" (CoreExpShow (CoreList as))
...
euCons whnfM [h, t] =
  whnfM t >>= \case
    ...
    e -> throwEvalError $ NotList (CoreExpShow e)
euCons _ as = throwEvalError $ Bug "__TAIL called with bad args" (CoreExpShow (CoreList as))
...
euSym whnfM [s] = whnfM s >>= \case
  ...
  e -> throwEvalError $ SymbolNamesMustBeStrings (CoreExpShow e)
euSym _ as = throwEvalError $ Bug "__SYM called with bad args" (CoreExpShow (CoreList as))
...
euBlock _ as = throwEvalError $ Bug "__BLOCK called with bad args" (CoreExpShow (CoreList as))
...
euElements whnfM [e] =
  whnfM e >>= \case
    ...
    _ -> throwEvalError $ ElementsArgumentNotBlock (CoreExpShow e)
euElements _ as = throwEvalError $ Bug "__ELEMENTS called with bad args" (CoreExpShow (CoreList as))
...
mergeElements  whnfM l r =
  ...
  where pairs = mapM (whnfM >=> pair) (l ++ r)
        ...
        pair el = throwEvalError $ BadBlockElement (CoreExpShow el)
  ...
  case (l', r') of
    (CoreBlock l'', CoreBlock r'') -> do
      ...
      case (l''', r''') of
        ...
        _ -> throwEvalError $ BadBlockMerge (CoreExpShow (CoreList [l, r]))
    _ -> throwEvalError $ BadBlockMerge (CoreExpShow (CoreList [l, r]))
euMerge _ as = throwEvalError $ Bug "Bad arguments for __MERGE" (CoreExpShow (CoreList as))
...
unwrapBlockElement whnfM expr@(CoreList [k, v]) = do
  ...
  case key of
    ...
    _ -> throwEvalError $ BadBlockElement (CoreExpShow expr)
unwrapBlockElement _ expr = throwEvalError $ BadBlockElement (CoreExpShow expr)
...
buildSearchList whnfM (CoreBlock list) =
  whnfM list >>= skipMeta whnfM >>= \case
    (CoreList items) -> mapM eval (reverse items)
    e -> throwEvalError $ LookupTargetNotList (CoreExpShow e)
  where
    eval item = whnfM item >>= skipMeta whnfM >>= unwrapBlockElement whnfM
buildSearchList _ e = throwEvalError $ LookupTargetNotList (CoreExpShow e)
...
euLookupOr whnfM [n, d, b] = do
  ...
  case n' of
    ...
    _ -> throwEvalError $ LookupKeyNotStringLike (CoreExpShow n')
euLookupOr _ as = throwEvalError $ Bug "__LOOKUP called with bad arguments" (CoreExpShow (CoreList as))
...
euLookup whnfM [n, b] = do
  ...
  case n' of
    ...
    _ -> throwEvalError $ LookupKeyNotStringLike (CoreExpShow n')
euLookup _ as = throwEvalError $ Bug "__LOOKUP called with bad arguments" (CoreExpShow (CoreList as))
...
removeItem whnfM b name = do
  ...
  case b' of
    (CoreBlock l) -> do
      ...
      case items of
        ...
        _ -> throwEvalError $ BadBlockContent (CoreExpShow l)
    _ -> throwEvalError $ RemoveArgumentNotBlock (CoreExpShow b)
  where mismatch item = whnfM item >>= \case
          ...
          _ -> throwEvalError $ LookupKeyNotStringLike (CoreExpShow item)
...
euRemove whnfM [n, b] = do
  ...
  case n' of
    ...
    _ -> throwEvalError $ LookupKeyNotStringLike (CoreExpShow n')
euRemove _ as = throwEvalError $ Bug "__LOOKUP called with bad arguments" (CoreExpShow (CoreList as))
...
euSplit whnfM [s, re] = do
  ...
  case (s', re') of
    ...
    _ -> throwEvalError $ BadSplitArgs (CoreExpShow s) (CoreExpShow re)
euSplit _ as = throwEvalError $ Bug "__SPLIT called with bad arguments" (CoreExpShow (CoreList as))
...
euJoin whnfM [l, sep] = do
  ...
  case (l', sep') of
    ...
    _ -> throwEvalError $ BadJoinArgs (CoreExpShow l') (CoreExpShow sep')
  where
    stringItem x = whnfM x >>= extractString
    extractString (CorePrim (CoreString x)) = return x
    extractString x = throwEvalError $ NotString (CoreExpShow x)
euJoin _ as = throwEvalError $ Bug "__JOIN called with bad arguments" (CoreExpShow (CoreList as))
...
euMatch whnfM [s, re] = do
  ...
  case (s', re') of
    ...
    _ -> throwEvalError $ BadMatchArgs (CoreExpShow s) (CoreExpShow re)
euMatch _ as = throwEvalError $ Bug "__MATCH called with bad arguments" (CoreExpShow (CoreList as))
...
euMatches whnfM [s, re] = do
  ...
  case (s', re') of
    ...
    _ -> throwEvalError $ BadMatchArgs (CoreExpShow s) (CoreExpShow re)
euMatches _ as = throwEvalError $ Bug "__MATCHES called with bad arguments" (CoreExpShow (CoreList as))
...
euWithMeta _ as =
  throwEvalError $ Bug "__WITHMETA called with bad arguments" (CoreExpShow (CoreList as))
...
euStr _ as =
  throwEvalError $ Bug "__STR called with bad arguments" (CoreExpShow (CoreList as))
...
euCat _ as = throwEvalError $ Bug "__CAT called with bad arguments" (CoreExpShow (CoreList as))
